...
const server = http.createServer((req, res) => {
  ...
  if (pathname === '/api/reports' && req.method === 'POST') {
    return collectBody(req, (err, body) => {
      if (err) return sendJSON(res, 400, { error: 'Invalid JSON body' });
      const { room, message, email, id } = body;
      if (!message || !String(message).trim()) return sendJSON(res, 400, { error: 'Message is required' });
      if (!email || !String(email).trim()) return sendJSON(res, 400, { error: 'Email is required' });

      const data = readData();
      const list = (!room || room === 'cafeteria')
        ? data.cafeteria.comments
        : (data.rooms[String(room).trim().toLowerCase()] || {}).reports;

      if (room && room !== 'cafeteria' && !data.rooms[String(room).trim().toLowerCase()]) {
        return sendJSON(res, 404, { error: 'Room not found' });
      }

      if (id) {
        // Edit an existing comment/report — only its original author may edit it.
        const existing = (list || []).find((r) => r.id === id);
        if (!existing) return sendJSON(res, 404, { error: 'Comment not found' });
        if (existing.email !== normalizeEmail(email)) return sendJSON(res, 403, { error: 'You can only edit your own comments' });
  ...
  if (pathname === '/api/reports' && req.method === 'DELETE') {
    return collectBody(req, (err, body) => {
      if (err) return sendJSON(res, 400, { error: 'Invalid JSON body' });
      const { room, id, email } = body;
      if (!id) return sendJSON(res, 400, { error: 'Comment id is required' });
      if (!email || !String(email).trim()) return sendJSON(res, 400, { error: 'Email is required' });

      const data = readData();
      const isCafeteria = !room || room === 'cafeteria';
      const key = isCafeteria ? null : String(room).trim().toLowerCase();
      if (!isCafeteria && !data.rooms[key]) return sendJSON(res, 404, { error: 'Room not found' });
      ...
      if (list[index].email !== normalizeEmail(email)) return sendJSON(res, 403, { error: 'You can only delete your own comments' });
      list.splice(index, 1);
