...
(function () {
  ...
  // Comments/reports created before edit/delete existed have no `id`,
  // so editing/deleting them silently failed. Backfill on read.
  function backfillIds(rooms, cafeteria) {
    let changed = false;
    (cafeteria.comments || []).forEach((c) => {
  ...
  const localApi = {
    ...
    postReport(payload) {
      const rooms = readLocal(LOCAL_ROOMS_KEY, {});
      const cafeteria = readLocal(LOCAL_CAFETERIA_KEY, emptyCafeteria());
      const isCafeteria = !payload.room || payload.room === 'cafeteria';
      const key = isCafeteria ? null : payload.room.trim().toLowerCase();
      if (!isCafeteria && !rooms[key]) return Promise.reject(new Error('Room not found'));
      const list = isCafeteria ? (cafeteria.comments = cafeteria.comments || []) : (rooms[key].reports = rooms[key].reports || []);
      const email = payload.email.trim().toLowerCase();

      if (payload.id) {
        const existing = list.find((r) => r.id === payload.id);
        if (!existing) return Promise.reject(new Error('Comment not found'));
        if (existing.email !== email) return Promise.reject(new Error('You can only edit your own comments'));
        existing.message = String(payload.message).trim().slice(0, 300);
        existing.editedAt = Date.now();
      } else {
        const entry = { id: crypto.randomUUID(), message: String(payload.message).trim().slice(0, 300), email, timestamp: Date.now() };
        list.unshift(entry);
        if (isCafeteria) cafeteria.comments = list.slice(0, 30);
        else rooms[key].reports = list.slice(0, 20);
      }

      if (isCafeteria) writeLocal(LOCAL_CAFETERIA_KEY, cafeteria);
      else writeLocal(LOCAL_ROOMS_KEY, rooms);
      const streak = bumpStreakLocal(payload.email);
    ...
    deleteReport(payload) {
      ...
      const key = isCafeteria ? null : payload.room.trim().toLowerCase();
      if (!isCafeteria && !rooms[key]) return Promise.reject(new Error('Room not found'));
      const list = isCafeteria ? (cafeteria.comments || []) : (rooms[key].reports || []);
      const email = payload.email.trim().toLowerCase();
      const index = list.findIndex((r) => r.id === payload.id);
      if (index === -1) return Promise.reject(new Error('Comment not found'));
      if (list[index].email !== email) return Promise.reject(new Error('You can only delete your own comments'));
      list.splice(index, 1);

      if (isCafeteria) { cafeteria.comments = list; writeLocal(LOCAL_CAFETERIA_KEY, cafeteria); }
      else { rooms[key].reports = list; writeLocal(LOCAL_ROOMS_KEY, rooms); }
      return Promise.resolve({ rooms: Object.values(rooms).sort((a, b) => b.timestamp - a.timestamp), cafeteria });
    },
    getCafeteria() {
      const rooms = readLocal(LOCAL_ROOMS_KEY, {});
      const cafeteria = readLocal(LOCAL_CAFETERIA_KEY, emptyCafeteria());
      backfillIds(rooms, cafeteria);
      return Promise.resolve(cafeteria);
    },
  ...
  const serverApi = {
    ...
    deleteReport(payload) {
      return fetch('/api/reports', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      }).then((r) => r.json().then((d) => { if (!r.ok) throw new Error(d.error || 'Could not delete comment'); return d; }));
    },
    getCafeteria() {
  ...
  function startEditReport(id, currentMessage) {
    state.editingReportId = id;
    state.editReportDraft = currentMessage;
    render();
  }

  function cancelEditReport() {
    state.editingReportId = null;
    state.editReportDraft = '';
    render();
  }

  function saveEditReport(roomKey, id) {
    const message = state.editReportDraft.trim();
    if (!message || state.editReportSubmitting) return;
    state.editReportSubmitting = true;
    render();
    api.postReport({ room: roomKey === 'cafeteria' ? 'cafeteria' : findRoomNameByKey(roomKey), message, email: state.session.email, id })
      .then((data) => {
        state.rooms = data.rooms;
        if (data.cafeteria) state.cafeteria = data.cafeteria;
        state.editingReportId = null;
        state.editReportDraft = '';
        state.editReportSubmitting = false;
        render();
        showToast(t('toastCommentUpdated'));
      })
      .catch((err) => {
        state.editReportSubmitting = false;
        render();
  ...
  function deleteReportEntry(roomKey, id) {
    if (!window.confirm(t('confirmDeleteComment'))) return;
    state.deletingReportId = id;
    render();
    api.deleteReport({ room: roomKey === 'cafeteria' ? 'cafeteria' : findRoomNameByKey(roomKey), id, email: state.session.email })
      .then((data) => {
        state.rooms = data.rooms;
        if (data.cafeteria) state.cafeteria = data.cafeteria;
  ...
  function findReportMessage(roomKey, id) {
    const list = roomKey === 'cafeteria'
      ? (state.cafeteria.comments || [])
      : ((state.rooms.find((r) => r.name.toLowerCase() === roomKey) || {}).reports || []);
  ...
  function renderCommentItem(entry, roomKey) {
    const isMine = state.session && entry.email === state.session.email;
    const isEditing = state.editingReportId === entry.id;
    const isDeleting = state.deletingReportId === entry.id;

    if (isEditing) {
      return `
        <div class="report-item report-item-editing">
          <input type="text" data-edit-report-input="${escapeHtml(entry.id)}" value="${escapeHtml(state.editReportDraft)}" maxlength="300" />
          <div class="report-item-edit-actions">
            <button class="btn-primary btn-tiny" data-action="save-edit-report" data-room="${escapeHtml(roomKey)}" data-id="${escapeHtml(entry.id)}" ${(!state.editReportDraft.trim() || state.editReportSubmitting) ? 'disabled' : ''}>${state.editReportSubmitting ? '…' : escapeHtml(t('saveStatusBtn'))}</button>
            <button class="btn-ghost btn-tiny" data-action="cancel-edit-report">${escapeHtml(t('cancelBtn'))}</button>
          </div>
        </div>
      `;
    }
    ...
    return `
      <div class="report-item">
        <span class="report-email">👤 ${escapeHtml(entry.email)}</span>
        <span class="report-msg">${escapeHtml(entry.message)}${entry.editedAt ? ` <em class="edited-tag">${escapeHtml(t('editedTag'))}</em>` : ''}</span>
        <span class="report-time">${timeAgo(entry.timestamp)}</span>
        ${isMine ? `
          <span class="report-item-actions">
            <button class="btn-ghost btn-tiny" data-action="edit-report" data-room="${escapeHtml(roomKey)}" data-id="${escapeHtml(entry.id)}">${escapeHtml(t('editBtn'))}</button>
            <button class="btn-ghost btn-tiny btn-danger" data-action="delete-report" data-room="${escapeHtml(roomKey)}" data-id="${escapeHtml(entry.id)}" ${isDeleting ? 'disabled' : ''}>${escapeHtml(t('deleteBtn'))}</button>
          </span>
        ` : ''}
      </div>
    `;
        ...
        ${open ? `
          <div class="report-list">
            ${reports.length === 0
              ...
              : reports.slice(0, 5).map((r) => renderCommentItem(r, key)).join('')}
          </div>
        ...
        ${open ? `
          <div class="report-list">
            ${comments.length === 0
              ...
              : comments.slice(0, 8).map((c) => renderCommentItem(c, 'cafeteria')).join('')}
          </div>
          <div class="report-form">
  ...
  function render() {
    ...
    if (state.editingReportId) {
      const el = root.querySelector('[data-edit-report-input]');
      if (el && document.activeElement !== el) {
        el.focus();
        el.setSelectionRange(el.value.length, el.value.length);
      }
  ...
  function handleClick(e) {
    ...
    else if (action === 'edit-report') { startEditReport(btn.dataset.id, findReportMessage(btn.dataset.room, btn.dataset.id)); }
    else if (action === 'cancel-edit-report') { cancelEditReport(); }
    else if (action === 'save-edit-report') { saveEditReport(btn.dataset.room, btn.dataset.id); }
    else if (action === 'delete-report') { deleteReportEntry(btn.dataset.room, btn.dataset.id); }
  ...
  function handleInputOrChange(e) {
    ...
    if (t.dataset.editReportInput !== undefined) {
      state.editReportDraft = t.value;
      const saveBtn = root.querySelector(`[data-action="save-edit-report"][data-id="${CSS.escape(t.dataset.editReportInput)}"]`);
      if (saveBtn) saveBtn.disabled = !t.value.trim() || state.editReportSubmitting;
    }
  }
